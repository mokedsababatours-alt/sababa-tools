...
function GaugeBar({ percent }: { percent: number }) {
  const color =
    percent > 85 ? "var(--color-coral)" : percent > 60 ? "var(--color-gold)" : "var(--color-teal)";
  return (
        ...
        {config.load && stats.load && (
          <StatCard title="Load Average">
            <div className="flex gap-4 mt-1">
              {[["1m", stats.load.one], ["5m", stats.load.five], ["15m", stats.load.fifteen]].map(
                ([label, val]) => (
    ...
    <div className="min-h-screen bg-[var(--background)] text-[var(--foreground)] p-6">
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex gap-2">
          ...
          {isAdmin && (
            <button
              onClick={() => { setDraftConfig(config); setConfigOpen(true); }}
